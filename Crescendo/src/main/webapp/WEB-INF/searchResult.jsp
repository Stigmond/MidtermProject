	...
	<jsp:attribute name="body">
		<c:forEach var="c" items="${ resultList }">
			...
			<p>(${ c.blogComments.size() } replies ) }</p>
				...
				<c:when test="${c.getClass().getSimpleName().equals(\"Trade\") }">
					<h5><a href="tradePage.do?id=${c.id }">${c.title }</a></h5>
					<em>listed by <a href="getUserProfile.do?id=${ c.user.id }">${ c.user.username } </a> at ${ c.createdAt }</em><br>
					<p><em>${ c.getSample() }...</em></p>
				</c:when>
			</c:choose>
		</c:forEach>
